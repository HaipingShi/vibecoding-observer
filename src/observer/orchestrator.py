        ...
        for adapter, session_paths in self._select_adapters(discovery):
            for path in session_paths:
                for event in adapter.parse(path):
                    if self.project_path is None or _event_in_project(event, self.project_path):
                        events.append(event)
        return events
...
def _normalize_project_path(path: str | Path) -> Path:
    return Path(path).expanduser().resolve()


def _event_in_project(event: IREvent, project_path: Path) -> bool:
    if not event.cwd:
        return False
